...
app.post("/api/users", (req, res, _) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const parsed = CreateUserSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ error: zod_1.z.treeifyError(parsed.error) });
            return;
        }
        const { first_name, email, last_name, active, username, country } = parsed.data;
...
app.get("/api/users/duplicates", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { limit = "50", offset = "0" } = req.query;
        const duplicates = yield prisma.user.groupBy({
...
const STATE_COORDS = {
    ...
    KY: { lat: 37.668140, lon: -84.670067 }, LA: { lat: 31.169960, lon: -91.867805 },
...
app.get("/api/weather/:state", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const pointsRes = yield fetch(`https://api.weather.gov/points/${coords.lat},${coords.lon}`, { headers: { "User-Agent": "nectar-technical-assessment" } });
        if (!pointsRes.ok) {
            res.status(502).json({ error: "Failed to reach National Weather Service" });
            return;
        }
        const pointsData = yield pointsRes.json();
        const forecastRes = yield fetch(pointsData.properties.forecast, {
            headers: { "User-Agent": "nectar-technical-assessment" },
        });
        if (!forecastRes.ok) {
            res.status(502).json({ error: "Failed to fetch forecast from National Weather Service" });
            return;
        }
